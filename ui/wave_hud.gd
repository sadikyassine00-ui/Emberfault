...
func _ready() -> void:
	...
	if not director:
		# Auto-discover Director in current scene if not explicitly assigned
		director = get_tree().current_scene.find_child("HordeDirector", true, false) as DirectorStateController
		if not director:
			director = get_tree().current_scene.find_child("DirectorStateController", true, false) as DirectorStateController

	if director:
		# Freeze director immediately while HUD pre-start timer runs
	...
	# 3. Start Pre-Start Countdown sequence
...
func _animate_victory_banner() -> void:
	...
	# Continuous subtle floating / breathing pulse loop
	victory_tween.chain().tween_callback(func():
		if pulse_tween:
			pulse_tween.kill()
		pulse_tween = create_tween().set_loops()
		pulse_tween.tween_property(victory_banner, "scale", Vector2(1.05, 1.05), 1.2).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
		pulse_tween.tween_property(victory_banner, "scale", Vector2(1.0, 1.0), 1.2).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	)
